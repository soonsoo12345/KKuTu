(function() {
    ...
    */
    ...
    function gerror(code, txt, add) {
      var a = txt ? txt : L["error_" + code];
      var b = add ? add : "";
      //console.log(add);
      //console.log(b);
      showDialog($("#gerror")), $("#desc").html(a + b), $("#desc_code").val(code);
    }
    ...
    function applyOptions(a) {
        if(!a) $data.opts = { vb: 1, ve: 1 };
        else $data.opts = a;
        $data.vb = $data.opts.vb,
        $data.ve = $data.opts.ve,
        $("#volume-bgm").val($data.opts.vb),
        $("#volume-eff").val($data.opts.ve),
        ...
        $data.bgm && ($data.bgm.volume = $data.opts.vb, $data.bgm = playBGM($data.bgm.key, !0))
    }

    function applyFilters(a) {
      if(!a) $data.filters = { su: false, ow: false, ou: false };
      else $data.filters = a;
      $("#sort-user").attr("checked", $data.filters.su),
      $("#only-waiting").attr("checked", $data.filters.ow),
      $("#only-unlock").attr("checked", $data.filters.ou);
    }
    ...
    function onMessage(a) {
        ...
        switch (a.type) {
            case "recaptcha":
                var e = $("#intro-text");
                e.empty(), e.html("게스트는 캡챠 인증이 필요합니다.<br/>로그인을 하시면 캡챠 인증을 건너뛰실 수 있습니다.<br/><br/>"), e.append($('<div class="g-recaptcha" id="recaptcha" style="display: table; margin: 0 auto;"></div>')), grecaptcha.render("recaptcha", {
                    sitekey: a.siteKey,
                    callback: b
                });
                break;
            ...
            case "disconn":
                ...
                break;
            case "connRoom":
                $data._preQuick && (playSound("success"), $stage.dialog.quick.hide(), delete $data._preQuick), $stage.dialog.quick.hide(), $data.setUser(a.user.id, a.user), d = $data.usersR[a.user.id] = a.user, d.id == $data.id ? loading() : notice((d.profile.title || d.profile.name) + L.hasJoined), updateUserList();
                break;
            case "disconnRoom":
            ...
            case "dying":
                ...
                break;
            case "tail":
                notice(a.a + "|" + a.rid + "@" + a.id + ": " + (a.msg instanceof String ? a.msg : JSON.stringify(a.msg)), "tail");
                break;
            case "chat":
                a.notice ? notice(L["error_" + a.code] || a.value) : chat(a.profile || {
                    title: L.robot
            ...
            case "error":
                ...
                if(600 == a.code) alert(L["error_" + a.code] + "\n" + a.message)
                else gerror(a.code);
				if(410 == a.code) alert(L["error_" + a.code])
                else gerror(a.code);
				if(402 == a.code) alert(L["error_" + a.code])
                else gerror(a.code);
        }
    ...
    function addonNickname(a, b) {
        b.equip.NIK && a.addClass("x-" + b.equip.NIK), "b1_gm" == b.equip.BDG && a.addClass("x-gm")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "word" == b.equip.BDG && a.addClass("x-word")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "mg" == b.equip.BDG && a.addClass("x-mg")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "donate" == b.equip.BDG && a.addClass("x-donate")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "designer" == b.equip.BDG && a.addClass("x-designer")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "streamer" == b.equip.BDG && a.addClass("x-streamer")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "gm" == b.equip.BDG && a.addClass("x-gm1")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "garu" == b.equip.BDG && a.addClass("x-garu")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "league1" == b.equip.BDG && a.addClass("x-league1")
		b.equip.NIK && a.addClass("x-" + b.equip.NIK), "league2" == b.equip.BDG && a.addClass("x-league2")
    }
    ...
    function updateRoomList(a) {
        ...
        $stage.lobby.roomListTitle.html(L.RoomList.replace("FA{bars}", "<div style='float: left;'><i class='fa fa-bars'></i>") + " [" + d + L.GAE + "]" + '</div>' + "&nbsp;"), d ? ($(".rooms-gaming").css("display", $data.filters.ow ? "none" : ""), $(".rooms-locked").css("display", $data.filters.ou ? "none" : "")) : $stage.lobby.roomList.append($stage.lobby.createBanner.clone().on("click", b))
    }
    ...
    function requestRoomInfo(a) {
        ...
        $data._roominfo = a, $("#RoomInfoDiag .dialog-title").html(a + L.sRoomInfo), $("#ri-title").html((b.password ? "<i class='fa fa-lock'></i>&nbsp;" : "") + b.title), $("#ri-mode").html(L["mode" + MODE[b.mode]]), $("#ri-round").html(b.round + ", " + b.time + L.SECOND), $("#ri-limit").html(b.players.length + " / " + b.limit), b.players.forEach(function(a, d) {
            ...
            a = $data.users[a] || NULL_USER, b.players[d].robot ? (a.profile = {
                    ...
                    robot: !0
                }) : g.t = g.t || 0, c.append($("<div>").addClass("ri-player").append(f = $("<div>").addClass("moremi rip-moremi")).append(e = $("<div>").addClass("ellipse rip-title").html(a.profile.title || a.profile.name)).append($("<div>").addClass("rip-team team-" + g.t).html($("#team-" + g.t).html())).append($("<div>").addClass("rip-form").html(L["pform_" + g.f]))),

                a.id == b.master && e.prepend($("<label>").addClass("rip-master").html("[" + L.master + "]&nbsp;")), e.prepend(getLevelImage(a.data.score).addClass("profile-level rip-level")), renderMoremi(f, a.equip)
    ...
    function roundEnd(a, b) {
        ...
        $(".result-me-expl").empty(), $stage.game.display.html(L.roundEnd), $data._resultPage = 1, $data._result = null;
    ...
    function playSound(a, b) {
      var c, d, e = b ? $data.vb : $data.ve;
      d = $sound[a] || $sound.missing, window.hasOwnProperty("AudioBuffer") && d instanceof AudioBuffer ? (f = audioContext.createGain ? audioContext.createGain() : null, c = audioContext.createBufferSource(), c.startedAt = audioContext.currentTime, c.loop = b, c.buffer = d, f && (f.gain.value = e - 1, f.connect(audioContext.destination), c.connect(f)), c.connect(audioContext.destination)) : (d.readyState && (d.audio.currentTime = 0), d.audio.loop = b || false, d.audio.volume = e, c = d)
      return $_sound[a] = c, c.key = a, c.start(), c
    }
    ...
    function notice(a, b) {
        ...
        $('.tattle-agree').on('click', (a) => {
          $.post("/tattle/agree", {
              order: a.currentTarget.parentElement.className
          }, function(b) {
            a.currentTarget.parentElement.innerHTML = ""
          })
        })
        $('.tattle-nagree').on('click', (a) => {
          $.post("/tattle/nagree", {
              order: a.currentTarget.parentElement.className
          }, function(b) {
            a.currentTarget.parentElement.innerHTML = ""
    ...
    $(document).ready(function() {
        ...
        function h() {
            ws = new _WebSocket($data.URL), ws.onopen = function(a) {
                ...
                rws && rws.close(), stopAllSounds(), gerror(a.code, L.closed), $.get("/kkutu_notice.html", function(a) {
                    loading(a)
                })
        ...
        loadSounds($data._soundList, function() {
            ...
            if ($data.room && $data.room.gaming) return a.preventDefault(), !1
        }), $data.opts = $.cookie("kks"), $data.opts && applyOptions(JSON.parse($data.opts)), $data.filters = $.cookie("kksf"), $data.filters && applyFilters(JSON.parse($data.filters)), $(".dialog-head .dialog-title").on("mousedown", function(b) {
        ...
        for ($stage.menu.help.on("click", function(a) {
                ...
                if (nick.length > 10) return gerror('NICK_LENGTH_LIMIT', '닉네임은 10자 이하이어야 합니다.')
                else {
            ...
            }), $stage.dialog.gerrorOK.on("click", function(e) {
                $("#gerror").hide(), "601" != $("#desc_code").val() && "602" != $("#desc_code").val() || showDialog($stage.dialog.setNick)
            }), $stage.menu.sortS.on("click", function(e) {
                showDialog($stage.dialog.sortS)
            }), $stage.dialog.sortSOK.on("click", function(e) {
                applyFilters({
                  su: $("#sort-user").is(":checked"),
                  ow: $("#only-waiting").is(":checked"),
                  ou: $("#only-unlock").is(":checked")
                }), $.cookie("kksf", JSON.stringify($data.filters)),
                $stage.dialog.sortS.hide()
			...
			}), $stage.menu.league.on("click", function(a) {
                $("#league-board").attr("src", "/league.html"), showDialog($stage.dialog.league)
			}), $stage.menu.bulletin.on("click", function(a) {
                $("#bulletin-board").attr("src", "/tattle"), showDialog($stage.dialog.bulletin)
			}), $stage.menu.singo.on("click", function(a) {
                $("#singo-board").attr("src", "/support.html"), showDialog($stage.dialog.singo)
			...
			}), $stage.dialog.profileSingo.on("click", function(a) {
				$("#singo-board").attr("src", "/support.html"), showDialog($stage.dialog.singo)
        ...
        $data.room.game.title.length;
        clearBoard(), $data._roundTime = 1e3 * $data.room.time, $stage.game.display.html($data._char = "&lt;" + L["theme_" + a.theme] + "&gt;"), $stage.game.chain.show().html($data.chain = 0),

            $data.room.opts.mission && $stage.game.items.show().css("opacity", 1).html($data.mission = a.mission), drawRound(a.round), playSound("round_start"), recordEvent("roundReady", {
                data: a
            })
